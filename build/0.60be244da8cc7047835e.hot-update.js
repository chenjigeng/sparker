...
function handleBold(event, change) {
  ...
  if (prevChar === '*') {
    if (chars.match(/\**/gi).length > 2) {
      var firstIndex = chars.indexOf('**');
      var boldChars = chars.slice(firstIndex).replace('*', '');
      var range = chars.length - firstIndex;
      console.log(range);
      event.preventDefault();
      // change.delete();
      change.deleteBackward(range);
      change.insertText(boldChars);
...
//# sourceMappingURL=0.60be244da8cc7047835e.hot-update.js.map