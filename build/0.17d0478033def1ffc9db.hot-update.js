  ...
  if (prevChar === '*') {
    if (chars.match(/\*\*/gi) && chars.match(/\*\*/gi).length >= 1) {
      ...
      var boldChars = void 0,
          moveCursorLength = void 0,
          range = void 0;
      if (firstIndex > startOffset) {
        boldChars = chars.slice(startOffset, firstIndex);
        moveCursorLength = boldChars.length;
        range = firstIndex - startOffset + 3;
      } else {
        boldChars = chars.splice(firstIndex).replace(/\*/gi, '');
        moveCursorLength = -boldChars.length;
        range = chars.length - firstIndex;
      }
      console.log(range);
      event.preventDefault();
      // change.delete();
      change.deleteBackward(range);
      change.insertText(boldChars).extend(moveCursorLength).addMark('bold').collapseToEnd().removeMark('bold');
...
//# sourceMappingURL=0.17d0478033def1ffc9db.hot-update.js.map