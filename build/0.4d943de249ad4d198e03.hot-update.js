...
function handleBold(event, change) {
  ...
  if (isToggleMark('*', chars, startOffset)) {
    ...
    // change.delete();
    change.deleteForward(deleteRange).insertText(boldChars).extend(-boldChars.length).addMark('bold').collapseToEnd().removeMark('bold');
...
function findDeleteRangeAndTargetChars(change, chars, startOffset) {
  ...
  if (startOffset > firstIndex + 1) {
    ...
    if (chars[startOffset] === '*') {
      deleteRange = startOffset - firstIndex + 2;
...
//# sourceMappingURL=0.4d943de249ad4d198e03.hot-update.js.map